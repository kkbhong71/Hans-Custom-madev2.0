...
class BrotherLottoSystem:
    ...
    def run(self, windows=None, output_count=21):
        if windows is None:
            windows = [30, 50, 100]
        ...
        for w in windows:
            ...
            for code, name in self.ALGO_NAMES:
                ...
                if nums and len(nums) == 6:
                    all_candidates.append((code, name, nums, w))

        # Extra candidates
        extra_count = max(0, output_count * 2 - len(all_candidates))
        if extra_count > 0:
            for w in windows:
                if w > len(self.df): continue
                pool_data = self.pool_analyzer.analyze(w)
                ...
                for _ in range(extra_count // len(windows) + 1):
                    algo = random.choice(['B', 'C', 'E', 'F'])
                    nums = self.generator.generate(
                        algo, hot_pool=hp, hot_weights=hw,
                        cold_pool=pool_data['cold_pool'], warm_pool=pool_data['warm_pool']
                    )
                    if nums and len(nums) == 6:
                        all_candidates.append((algo, dict(self.ALGO_NAMES).get(algo, ''), nums, w))

        optimized = CoverageOptimizer.optimize(all_candidates, output_count)
        ...
        uncovered = sorted(set(range(1, 46)) - all_numbers)
...
def api_predict():
    ...
    output_count = data.get('output_count', 21)
    results = system.run(windows=windows, output_count=output_count)
