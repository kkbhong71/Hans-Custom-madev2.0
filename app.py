...
class BrotherLottoSystem:
    ...
    def run(self, windows=None, sets_per_window=7):
        """
        윈도우별 독립 실행: 각 구간에서 sets_per_window 세트씩 생성
        Returns: { windows_data: { 30: {...}, 50: {...}, 100: {...} }, global_stats: {...} }
        """
        if windows is None:
            windows = [30, 50, 100]
        ...
        for w in windows:
            ...
            # 추가 후보 생성 (sets_per_window > 7 일 때)
            extra_needed = max(0, sets_per_window * 2 - len(candidates))
            if extra_needed > 0:
            ...
            # 구간별 커버리지 최적화
            optimized = CoverageOptimizer.optimize(candidates, sets_per_window)
